...
pimSimConfig::show() const
{
  ...
  if (m_bufferSize > 0) std::printf(", bufferSize = %u bits", m_bufferSize);
  std::printf("\n");
  if (m_vectorRegisterCount > 0) {
    std::printf("PIM-Config: Vector Register Count = %u, Bit Width = %u\n", m_vectorRegisterCount, m_vectorRegisterBitWidth);
  }
  if (m_scalarRegisterCount > 0) {
    std::printf("PIM-Config: Scalar Register Count = %u, Bit Width = %u\n", m_scalarRegisterCount, m_scalarRegisterBitWidth);
  }
...
//! @brief  Derive Params: Register Config
bool
pimSimConfig::deriveRegisterConfig()
{
  m_vectorRegisterCount = 0;
  m_vectorRegisterBitWidth = 0;
  m_scalarRegisterCount = 0;
  m_scalarRegisterBitWidth = 0;

  bool hasVal = false;
  std::string valStr = pimUtils::getOptionalParam(m_cfgParams, m_cfgVarVectorRegisterCount, hasVal);
  if (hasVal) {
    bool ok = pimUtils::convertStringToUnsigned(valStr, m_vectorRegisterCount);
    if (!ok || m_vectorRegisterCount == 0) {
      std::printf("PIM-Error: Incorrect config file parameter: %s=%s\n", m_cfgVarVectorRegisterCount.c_str(), valStr.c_str());
      return false;
    }
  } else {
    valStr = pimUtils::getOptionalParam(m_envParams, m_envVarVectorRegisterCount, hasVal);
    if (hasVal) {
      bool ok = pimUtils::convertStringToUnsigned(valStr, m_vectorRegisterCount);
      if (!ok || m_vectorRegisterCount == 0) {
        std::printf("PIM-Error: Incorrect environment variable: %s=%s\n", m_envVarVectorRegisterCount.c_str(), valStr.c_str());
        return false;
      }
    }
  }
  if (m_vectorRegisterCount > 0) {
    valStr = pimUtils::getOptionalParam(m_cfgParams, m_cfgVarVectorRegisterBitWidth, hasVal);
    if (hasVal) {
      bool ok = pimUtils::convertStringToUnsigned(valStr, m_vectorRegisterBitWidth);
      if (!ok || m_vectorRegisterBitWidth == 0) {
        std::printf("PIM-Error: Incorrect config file parameter: %s=%s\n", m_cfgVarVectorRegisterBitWidth.c_str(), valStr.c_str());
        return false;
      }
    } else {
      valStr = pimUtils::getOptionalParam(m_envParams, m_envVarVectorRegisterBitWidth, hasVal);
      if (hasVal) {
        bool ok = pimUtils::convertStringToUnsigned(valStr, m_vectorRegisterBitWidth);
        if (!ok || m_vectorRegisterBitWidth == 0) {
          std::printf("PIM-Error: Incorrect environment variable: %s=%s\n", m_envVarVectorRegisterBitWidth.c_str(), valStr.c_str());
          return false;
        }
      }
    }
  }
  if ((m_vectorRegisterCount > 0 && m_vectorRegisterBitWidth == 0) || (m_vectorRegisterCount == 0 && m_vectorRegisterBitWidth > 0)) {
    std::printf("PIM-Error: Vector register count and bit width must be specified together.\n");
    return false;
  }

  // Scalar registers
  valStr = pimUtils::getOptionalParam(m_cfgParams, m_cfgVarScalarRegisterCount, hasVal);
  if (hasVal) {
    bool ok = pimUtils::convertStringToUnsigned(valStr, m_scalarRegisterCount);
    if (!ok || m_scalarRegisterCount == 0) {
      std::printf("PIM-Error: Incorrect config file parameter: %s=%s\n", m_cfgVarScalarRegisterCount.c_str(), valStr.c_str());
      return false;
    }
  } else {
    valStr = pimUtils::getOptionalParam(m_envParams, m_envVarScalarRegisterCount, hasVal);
    if (hasVal) {
      bool ok = pimUtils::convertStringToUnsigned(valStr, m_scalarRegisterCount);
      if (!ok || m_scalarRegisterCount == 0) {
        std::printf("PIM-Error: Incorrect environment variable: %s=%s\n", m_envVarScalarRegisterCount.c_str(), valStr.c_str());
        return false;
      }
    }
  }
  ...
  if (m_scalarRegisterCount > 0) {
    valStr = pimUtils::getOptionalParam(m_cfgParams, m_cfgVarScalarRegisterBitWidth, hasVal);
    if (hasVal) {
      bool ok = pimUtils::convertStringToUnsigned(valStr, m_scalarRegisterBitWidth);
      if (!ok || m_scalarRegisterBitWidth == 0) {
        std::printf("PIM-Error: Incorrect config file parameter: %s=%s\n", m_cfgVarScalarRegisterBitWidth.c_str(), valStr.c_str());
        return false;
      }
    } else {
      valStr = pimUtils::getOptionalParam(m_envParams, m_envVarScalarRegisterBitWidth, hasVal);
      if (hasVal) {
        bool ok = pimUtils::convertStringToUnsigned(valStr, m_scalarRegisterBitWidth);
        if (!ok || m_scalarRegisterBitWidth == 0) {
          std::printf("PIM-Error: Incorrect environment variable: %s=%s\n", m_envVarScalarRegisterBitWidth.c_str(), valStr.c_str());
          return false;
        }
      }
    }
  }
  if (m_scalarRegisterCount > 0 && m_scalarRegisterBitWidth == 0) {
    std::printf("PIM-Error: Scalar register bit width must be specified when scalar register count is specified.\n");
    return false;
  } else if (m_scalarRegisterCount == 0 && m_scalarRegisterBitWidth > 0) {
    std::printf("PIM-Error: Scalar register count must be specified when scalar register bit width is specified.\n");
    return false;
  } else if (m_scalarRegisterCount > 0 && m_scalarRegisterBitWidth > 0) {
    std::printf("PIM-Info: Scalar Register Count = %u, Bit Width = %u\n", m_scalarRegisterCount, m_scalarRegisterBitWidth);
  }
  return true;
}
