...
pimPerfEnergyBitSerial::getPerfEnergyForPIMProg(std::vector<pimeval::cmdNode>& cmdGraph) const
{
  std::vector<pimeval::perfEnergy> perfEnergies(cmdGraph.size(), pimeval::perfEnergy());
  for (const auto& cmdNode : cmdGraph) {
    PimCmdEnum cmdType = cmdNode.cmdType;
    switch (cmdType) {
    case PimCmdEnum::BROADCAST:
    {
      perfEnergies[cmdNode.cmdId] = getPerfEnergyForBroadcast(cmdType, *cmdNode.dests[0]);
      break;
    }
    case PimCmdEnum::REDMAX:
    case PimCmdEnum::REDMAX_RANGE:
    ...
    case PimCmdEnum::REDSUM_RANGE:
    {
      perfEnergies[cmdNode.cmdId] = getPerfEnergyForReduction(cmdType, *cmdNode.srcs[0], 1);
      break;
    }
    case PimCmdEnum::ROTATE_ELEM_L:
    case PimCmdEnum::ROTATE_ELEM_R:
    case PimCmdEnum::SHIFT_ELEM_L:
    case PimCmdEnum::SHIFT_ELEM_R:
    {
      perfEnergies[cmdNode.cmdId] = getPerfEnergyForRotate(cmdType, *cmdNode.srcs[0]);
      break;
    }
    case PimCmdEnum::ADD:
    case PimCmdEnum::SUB:
    ...
    {
      perfEnergies[cmdNode.cmdId] = getPerfEnergyForFunc2(cmdType, *cmdNode.srcs[0], *cmdNode.srcs[1], *cmdNode.dests[0]);
      break;
    }
    case PimCmdEnum::COPY_O2O:
    ...
    case PimCmdEnum::SHIFT_BITS_R:
    {
      perfEnergies[cmdNode.cmdId] = getPerfEnergyForFunc1(cmdType, *cmdNode.srcs[0], *cmdNode.dests[0]);
      break;
    }
    case PimCmdEnum::CONVERT_TYPE:
    {
      // handle type conversion
      perfEnergies[cmdNode.cmdId] = getPerfEnergyTypeConversion(m_simTarget, cmdType, *cmdNode.srcs[0], *cmdNode.dests[0]);
      break;
    }
    ...
    {
      // handle bytes transfer
      perfEnergies[cmdNode.cmdId] = pimeval::perfEnergy(0, 0, 0, 0, 0, 0);
      break;
    }
    default:
      break;
