...
pimResMgr::pimAllocBuffer(uint32_t numElements, PimDataType dataType)
{
  ...
  if (numElements * bitsPerElement > m_device->getBufferSize()) {
    printf("PIM-Error: pimAlloc: Invalid input parameter: %u elements exceeds buffer size %u bits\n",
  ...
  uint64_t numRegions = (numElements * bitsPerElement - 1) / numCols + 1; // AiM buffer size will be always size of one row; For UPMEM this will be different
  ...
  uint64_t elemIdx = 0;
  for (uint64_t i = 0; i < numRegions; ++i) {
    PimCoreId coreId = 0; // Global buffer is always assigned to core 0 as it is shared among all cores
    unsigned numColsToAlloc = (i == numRegions - 1 ? numColsToAllocLast : numCols);
    unsigned numElemInRegion = (i == numRegions - 1 ? numElemPerRegionLast : numElemPerRegion);
    pimRegion newRegion;
    newRegion.setCoreId(coreId);  // Assign global buffer to core 0; this is fine for global buffers; for UPMEM, this will be different
    newRegion.setRowIdx(i); // Each region is assigned to a different row in the global buffer
