    ...
    m_paramsDram(params.getParamsDram())
{
  ...
  m_eR_L = m_paramsDram.getPjRead_L() / m_pico_to_milli;
  m_eW_L = m_paramsDram.getPjWrite_L() / m_pico_to_milli;
  m_eR_S = m_paramsDram.getPjRead_S() / m_pico_to_milli;
  m_eW_S = m_paramsDram.getPjWrite_S() / m_pico_to_milli;
  ...
  m_tRCDRD = m_paramsDram.gettRCDRD();
  m_tRCDWR = m_paramsDram.gettRCDWR();
...
pimPerfEnergyBase::getPerfEnergyForBytesTransfer(PimCmdEnum cmdType, uint64_t numBytes) const
{
  ...
  switch (cmdType) {
    ...
    {
      mjEnergy = m_eW_L * msRuntime * m_numChipsPerRank * m_numRanks;
      mjEnergy += m_pBChip * m_numChipsPerRank * m_numRanks * msRuntime;
      break;
    }
    case PimCmdEnum::COPY_D2H:
    {
      mjEnergy = m_eR_L * msRuntime * m_numChipsPerRank * m_numRanks;
      mjEnergy += m_pBChip * m_numChipsPerRank * m_numRanks * msRuntime;
      break;
    }
...
pimeval::perfEnergy
pimPerfEnergyBase::getPerfEnergyForMac(PimCmdEnum cmdType, const pimObjInfo& objSrc, const pimObjInfo& objDest) const
{
  double msRuntime = 1e10;
  double mjEnergy = 999999999.9;
  double msRead = 0.0;
...
//! @brief  Perf energy model of base class for PIMProg
std::vector<pimeval::perfEnergy>
pimPerfEnergyBase::getPerfEnergyForPIMProg(std::vector<pimeval::cmdNode>& cmdGraph) const
{
  double msRuntime = 1e10;
  double mjEnergy = 999999999.9;
  double msRead = 0.0;
  ...
  double msCompute = 0.0;
  uint64_t mTotalOP = 0;
  return std::vector<pimeval::perfEnergy>(cmdGraph.size(), pimeval::perfEnergy(msRuntime, mjEnergy, msRead, msWrite, msCompute, mTotalOP));
}
