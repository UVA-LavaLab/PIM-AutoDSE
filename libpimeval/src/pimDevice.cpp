...
pimDevice::init()
{
  ...
  switch (m_config.getSimTarget())
  {
  case PIM_DEVICE_AQUABOLT:
  {
    auto aquaboltCfg = std::make_shared<pimeval::AquaboltConfig>();
    aquaboltCfg->grfWidth = m_config.getVectorRegisterBitWidth() > 0 ? m_config.getVectorRegisterBitWidth() : 256; // 256 bits per column
    aquaboltCfg->grfCount = m_config.getVectorRegisterCount() > 0 ? m_config.getVectorRegisterCount() : 16; // 16 GRFs
    aquaboltCfg->srfCount = m_config.getScalarRegisterCount() > 0 ? m_config.getScalarRegisterCount() : 16; // 16 SRFs
    aquaboltCfg->srfBitWidth = m_config.getScalarRegisterBitWidth() > 0 ? m_config.getScalarRegisterBitWidth() : 32; // 32 bits
    pimPerfEnergyModelParams params(PIM_DEVICE_AQUABOLT, getNumRanks(), paramsDram);
    params.setArchSpecificConfig(aquaboltCfg);
  ...
  case PIM_DEVICE_BANK_LEVEL:
  {
    ...
    bankLevelCfg->blimpScalarRegisterCount = m_config.getScalarRegisterCount(); // 0 means unlimited
