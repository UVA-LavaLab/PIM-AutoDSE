...
public:
  pimPerfEnergyAquabolt(const pimPerfEnergyModelParams& params) : pimPerfEnergyBase(params) {
    auto cfg = params.getArchSpecificConfig<pimeval::AquaboltConfig>();
    m_grfWidth = cfg->grfWidth;
    m_grfCount = cfg->grfCount;
    m_srfCount = cfg->srfCount;
  }
  ...
  virtual std::vector<pimeval::perfEnergy> getPerfEnergyForPIMProg(std::vector<pimeval::cmdNode>& cmdGraph) const override;

protected:
  unsigned m_aquaboltFPUBitWidth = 16;
  // TODO: Update for Aquabolt
  double m_aquaboltArithmeticEnergy = 0.0000000004992329586; // mJ
  unsigned m_grfWidth = 256; // Width of each Vector Register (in bits)
  unsigned m_grfCount = 16; // Number of Vector Registers per PIM core
  unsigned m_srfCount = 16; // Number of Scalar Registers per PIM core
private:
  void simulateExecution(std::vector<pimeval::cmdNode>& cmdGraph, std::vector<pimeval::perfEnergy> &perfEnergies, int maxBitsPerObject) const;
};
